    ...
    public static void constructConditionalProb() {
        ...
        for (Map.Entry<String, Integer> item_i : corpus.entrySet())
            for (Map.Entry<String, Integer> item_j : corpus.entrySet()) {
                ...
                String word_i = item_i.getKey();
                Integer wordId_i = vocab.get(word_i);
                Integer wordCount_i = item_i.getValue();

                // get the information of word j
                String word_j = item_j.getKey();
                Integer wordId_j = vocab.get(word_j);
                Integer wordCount_j = item_j.getValue();

                // determining the P(w_i | w_j)
                if (wordCount_j > 0) {
                    conditionalProbs[wordId_j][wordId_i] = jointProbs[wordId_j][wordId_i]
                            / ((double) wordCount_j / totalPairsOfWords);
                } else {
                    conditionalProbs[wordId_j][wordId_i] = 1e-10; // Tránh chia cho 0
                }

                // determining the P(w_j | w_i)
                if (wordCount_i > 0) {
                    conditionalProbs[wordId_i][wordId_j] = jointProbs[wordId_i][wordId_j]
                            / ((double) wordCount_i / totalPairsOfWords);
                } else {
                    conditionalProbs[wordId_i][wordId_j] = 1e-10; // Tránh chia cho 0
                }
            }
    }
